...
describe('Frame Logging Test', () => {
  ...
  it('should show frame updates with manual animation trigger', async () => {
    ...
    console.log('Frame VarNode:', { id: frameVarNode?.id, value: frameVarNode?.value?.read() });
    ...
    if (frameVarNode) {
      ...
      console.log('Global test callbacks:', (global as any).__testCallbacks?.length || 0);
      ...
      triggerAnimationFrame();
      console.log('Animation callbacks remaining after trigger 1:', animationCallbacks.length);
      triggerAnimationFrame();
      console.log('Animation callbacks remaining after trigger 2:', animationCallbacks.length);
