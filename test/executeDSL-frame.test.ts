...
describe("executeDSL Frame Integration Tests", () => {
  ...
  describe("Basic Frame Execution", () => {
    it("should execute frame() DSL successfully", () => {
      ...
      const watchSetupLogs = executionLog.filter(
        (log) => log.includes("watch") || log.includes("Watch"),
      );
    ...
    it("should detect multiple executions during single frame() call", () => {
      ...
      const originalConvertGraphToNodysseus =
        require("../src/graph-to-nodysseus-converter").convertGraphToNodysseus;
  ...
  describe("Graph Conversion and Caching", () => {
    ...
    it("should test graph contains frame detection", () => {
      ...
      console.log(
        "Contains extern.frame:",
        frameJsonString.includes("extern.frame"),
      );
  ...
  describe("Geometry Type Mutation Bug Reproduction", () => {
    it('should reproduce "Unknown geometry type: SphereGeometry" error on third loop', (done) => {
      console.log(
        "=== Test: Reproducing SphereGeometry type mutation error ===",
      );
      ...
      mockRequestAnimationFrame.mockImplementation((callback: Function) => {
        console.log(
          `=== RAF callback registered for frame update #${watchCallbacks.length + 1} ===`,
        );
            ...
            try {
              ...
              if (scene.children.length > 0) {
                ...
                if (mesh && mesh.geometry) {
                  ...
                  console.log(
                    "  - geometry object keys:",
                    Object.keys(mesh.geometry),
                  );
              ...
              if (watchCallbacks.length > 0) {
                ...
                console.log(
                  `Calling watch callback for frame ${frameUpdateCount}...`,
                );
                callback();
              }
              ...
              if (scene.children.length > 0) {
                ...
                if (mesh && mesh.geometry) {
                  ...
                  if (
                    typeof geometryTypeProperty !== "string" &&
                    geometryTypeProperty &&
                    geometryTypeProperty.constructor
                  ) {
                    ...
                    console.log(
                      ...
                      geometryTypeProperty.constructor.name,
                    );
                  }
                }
              }
      ...
      const runFrameUpdates = async () => {
        ...
        for (let i = 1; i <= 5; i++) {
          // Run 5 iterations to be sure we catch the error
          await triggerFrameUpdate(i);

          // Check if we've captured the specific error we're looking for
          const sphereGeometryError = capturedErrors.find(
            (e) =>
              e.message &&
              e.message.includes("Unknown geometry type: SphereGeometry"),
          );
        ...
        geometryTypeHistory.forEach((entry) => {
          console.log(
            `  ${entry.iteration}.${entry.phase}: ${entry.typeProperty} (string: ${entry.isString})`,
          );
        });

        // Test assertions - check both errors and warnings
        const targetError = capturedErrors.find(
          (e) =>
            e.message &&
            e.message.includes("Unknown geometry type: SphereGeometry"),
        );

        const targetWarning = capturedWarnings.find(
          (w) =>
            w.message &&
            w.message.includes("Unknown geometry type: SphereGeometry"),
        );

        if (targetError || targetWarning) {
          const found = targetError || targetWarning;
          console.log(
            '✅ Successfully reproduced the "Unknown geometry type: SphereGeometry" error',
          );
          console.log(
            `✅ Error occurred in ${targetError ? "error" : "warning"}`,
          );
          console.log("✅ Error details:", found);

          // Verify the error message is exactly what we expect
          expect(found.message).toContain(
            "Unknown geometry type: SphereGeometry",
          );
        } else {
          console.log("❌ Failed to reproduce the target error");
          console.log(
            "Captured errors:",
            capturedErrors.map((e) => e.message),
          );
          console.log(
            "Captured warnings:",
            capturedWarnings.map((w) => w.message),
          );

          // If we didn't get the exact error, let's at least verify we got some geometry type mutations
          const typeObjectMutations = geometryTypeHistory.filter(
            (entry) =>
              !entry.isString &&
              entry.typeProperty !== null &&
              entry.typeProperty !== undefined,
          );

          if (typeObjectMutations.length > 0) {
            console.log(
              "Found geometry type object mutations:",
              typeObjectMutations.length,
            );
  ...
  describe("Comprehensive Integration Test", () => {
    it("should run a complete frame animation scenario", async () => {
      ...
      expect(sceneObjectsAfter.length).toBeGreaterThan(
        sceneObjectsBefore.length,
      );
    });
  });
});
