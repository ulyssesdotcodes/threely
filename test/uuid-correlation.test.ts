...
import { generateUUIDTags, clearFunctionCallRegistry, getFunctionCallRegistry, getUUIDFromRangeSet, uuidRangeSetField, setUUIDRangeSet } from '../src/uuid-tagging';
...
describe('UUID Correlation System', () => {
  ...
  test('should generate UUIDs for function calls in RangeSet', () => {
    ...
    console.log('Function calls found:', functionCalls.map(fc => ({ name: fc.functionName, from: fc.from, to: fc.to })));
  ...
  test('should correlate UUIDs from RangeSet to Lezer nodes', () => {
    ...
    const { rangeSet, functionCalls } = generateUUIDTags(code);
    console.log('Generated function calls:', functionCalls.map(fc => ({ name: fc.functionName, uuid: fc.uuid, from: fc.from, to: fc.to })));
    
    // Convert with Lezer (should find and use the UUIDs)
    const result = convertLezerToNodysseus(code, dslContext);
    
    console.log('Conversion log entries:', result.conversionLog.map(entry => ({ 
    ...
    const matchingUuids = uuidsFromRangeSet.filter(uuid => uuidsFromConversion.includes(uuid));
  ...
  test('should handle complex chains with proper UUID assignment', () => {
    ...
    console.log('Complex chain function calls:', functionCalls.map(fc => ({ name: fc.functionName, from: fc.from, to: fc.to })));
  ...
  test('should map RangeSet through document changes', () => {
    ...
    const { rangeSet: initialRangeSet, functionCalls: initialCalls } = generateUUIDTags(initialCode);
    ...
    const uuidFromState = getUUIDFromRangeSet(stateWithUuids.field(uuidRangeSetField), 0);
    ...
    const unmappedUuid = getUUIDFromRangeSet(changedState.field(uuidRangeSetField), 0);
