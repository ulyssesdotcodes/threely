...
describe("Lezer to Nodysseus Converter", () => {
  it("should convert simple number literal", () => {
    ...
    expect(
      result.conversionLog.some((entry) => entry.astNodeType === "Number"),
    ).toBe(true);
  });
  ...
  it("should debug complex method chaining expression", () => {
    ...
    console.log(
      "Total edges:",
      Object.keys(result.graph.edges_in || {}).length,
    );
    ...
    Object.entries(result.graph.nodes).forEach(([nodeId, node]) => {
      if ("ref" in node) {
        console.log(
          `  ${nodeId}: RefNode(${node.ref}) = ${typeof node.value === "function" ? "[Function]" : node.value}`,
        );
      } else {
    ...
    expectedFunctions.forEach((fn) => {
      ...
      if (!found) {
        console.log(
          `  Available functions in DSL context: ${Object.keys(dslContext)
            .filter((k) => typeof dslContext[k] === "function")
            .slice(0, 10)
            .join(", ")}...`,
        );
      }
  ...
  it("should compare Lezer converter vs eval-based approach", () => {
    ...
    if (evalResult && typeof evalResult === "object" && "id" in evalResult) {
      ...
      console.log(
        "Dependencies:",
        evalResult.dependencies?.map((dep: any) => dep.id) || [],
      );

      // Convert eval result to Nodysseus for comparison
      const {
        convertGraphToNodysseus,
      } = require("../src/graph-to-nodysseus-converter");
      const evalNodysseusGraph = convertGraphToNodysseus(evalResult);
      console.log(
        "Converted to Nodysseus - nodes:",
        Object.keys(evalNodysseusGraph.nodes).length,
      );
  ...
  it("should analyze detailed node structure differences", () => {
    ...
    Object.entries(lezerResult.graph.nodes).forEach(([nodeId, node]) => {
      ...
      if ("ref" in node) {
        ...
        console.log(
          `  Value: ${typeof node.value === "function" ? "[Function " + (node.value.name || "anonymous") + "]" : JSON.stringify(node.value)}`,
        );
      } else {
    ...
    Object.entries(evalNodysseusGraph.nodes).forEach(([nodeId, node]) => {
      ...
      if ((node as any).ref) {
        ...
        console.log(
          `  Value: ${typeof (node as any).value === "function" ? "[Function " + ((node as any).value.name || "anonymous") + "]" : JSON.stringify((node as any).value)}`,
        );
      } else {
        console.log(`  Type: ValueNode`);
    ...
    if (lezerResult.graph.edges_in) {
      Object.entries(lezerResult.graph.edges_in).forEach(
        ([toNodeId, edges]) => {
    ...
    if (evalNodysseusGraph.edges_in) {
      Object.entries(evalNodysseusGraph.edges_in).forEach(
        ([toNodeId, edges]) => {
          console.log(`${toNodeId} <-`);
    ...
    console.log(
      "2. Edge count: Lezer =",
      Object.keys(lezerResult.graph.edges_in || {}).length,
      ", Eval =",
      Object.keys(evalNodysseusGraph.edges_in || {}).length,
    );
    ...
    const lezerMethods = Object.values(lezerResult.graph.nodes).filter(
      (node: any) => "ref" in node && node.ref === "@dsl.method",
    ).length;
    const lezerFunctions = Object.values(lezerResult.graph.nodes).filter(
      (node: any) => "ref" in node && node.ref === "@dsl.function",
    ).length;

    console.log(
      `Lezer: ${lezerMethods} method calls, ${lezerFunctions} function calls`,
    );

    // Expected: sphere() and material() should be @dsl.function, others should be @dsl.method
    console.log(
      "Expected: sphere() and material() should be functions, mesh/translateX/rotateY/render should be methods",
    );
  ...
  it("should convert string literal", () => {
    ...
    expect(
      result.conversionLog.some((entry) => entry.astNodeType === "String"),
    ).toBe(true);
  });
  ...
  it("should convert simple function call", () => {
    ...
    expect(
      result.conversionLog.some(
        (entry) => entry.astNodeType === "CallExpression",
      ),
    ).toBe(true);
    expect(
      result.conversionLog.some((entry) => entry.functionResolved === "testFn"),
    ).toBe(true);
  });
  ...
  it("should handle unknown functions gracefully", () => {
    ...
    const callEntry = result.conversionLog.find(
      (entry) => entry.astNodeType === "CallExpression",
    );
    expect(callEntry).toBeDefined();
    expect(callEntry?.warnings?.some((w) => w.includes("not found"))).toBe(
      true,
    );
  });
  ...
  it("should convert DSL functions", () => {
    ...
    expect(
      result.conversionLog.some((entry) => entry.functionResolved === "mult"),
    ).toBe(true);
  });
  ...
  it("should handle method chaining", () => {
    ...
    expect(
      result.conversionLog.some((entry) => entry.functionResolved === "frame"),
    ).toBe(true);
    expect(
      result.conversionLog.some((entry) => entry.functionResolved === "mult"),
    ).toBe(true);
