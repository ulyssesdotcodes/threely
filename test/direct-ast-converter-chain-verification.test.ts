...
describe('Direct AST to Nodysseus Converter - Method Chain Edge Structure', () => {
  ...
  function analyzeChainStructure(graph: any, expression: string) {
    ...
    console.log(`📊 Graph Stats: ${nodes.length} nodes, ${Object.keys(edges).length} nodes with inputs`);
    ...
    Object.entries(graph.nodes).forEach(([nodeId, node]: [string, any]) => {
      ...
      const valueDesc = typeof node.value === 'function' 
    ...
    Object.entries(edges).forEach(([nodeId, nodeEdges]: [string, any]) => {
      ...
      const functionName = typeof node.value === 'function' ? node.value.name : 'unknown';
      console.log(`  ${nodeId} (${functionName}) ← [${dependencies.join(', ')}]`);
  ...
  function verifyChainSequence(analysis: any, expectedFunctions: string[]) {
    ...
    for (let i = 1; i < expectedFunctions.length; i++) {
      ...
      console.log(`🔗 ${currentFn} depends on ${prevFn}: ${hasPrevDependency ? '✅' : '❌'}`);
      
      if (!hasPrevDependency) {
        isValidChain = false;
        console.log(`   Current deps: [${Object.keys(currentDeps).join(', ')}]`);
      }
    }
  ...
  it('should create proper hierarchy for mesh(sphere(), material()).translateX(1).render("test")', () => {
    const expression = 'mesh(sphere(), material()).translateX(1).render("test")';
    console.log(`\n🧪 Testing: ${expression}`);
    ...
    const expectedFunctions = ['sphere', 'material', 'mesh', 'translateX', 'render'];
    const hasValidChain = verifyChainSequence(analysis, ['mesh', 'translateX', 'render']);
    ...
    const renderNodeId = Object.entries(analysis.nodes).find(([_, node]: [string, any]) => 
      'value' in node && typeof node.value === 'function' && node.value.name === 'render')?.[0];
    const translateXNodeId = Object.entries(analysis.nodes).find(([_, node]: [string, any]) => 
      'value' in node && typeof node.value === 'function' && node.value.name === 'translateX')?.[0];
    const meshNodeId = Object.entries(analysis.nodes).find(([_, node]: [string, any]) => 
      'value' in node && typeof node.value === 'function' && node.value.name === 'mesh')?.[0];
  ...
  it('should create proper hierarchy for mesh(sphere()).rotateY(45).translateX(1).render("test")', () => {
    const expression = 'mesh(sphere()).rotateY(45).translateX(1).render("test")';
    console.log(`\n🧪 Testing: ${expression}`);
    ...
    const maxDependencies = Math.max(...Object.values(analysis.edges).map((deps: any) => Object.keys(deps).length));
    console.log(`📊 Maximum dependencies on any single node: ${maxDependencies}`);
  ...
  it('should handle simple single calls correctly', () => {
    ...
    const functionNodes = Object.values(analysis.nodes).filter((node: any) => 
      'value' in node && typeof node.value === 'function');
  ...
  it('should handle mixed chains and single calls working together', () => {
    ...
    const sphereNodeId = Object.entries(analysis.nodes).find(([_, node]: [string, any]) => 
      'value' in node && typeof node.value === 'function' && node.value.name === 'sphere')?.[0];
    const materialNodeId = Object.entries(analysis.nodes).find(([_, node]: [string, any]) => 
      'value' in node && typeof node.value === 'function' && node.value.name === 'material')?.[0];
    const meshNodeId = Object.entries(analysis.nodes).find(([_, node]: [string, any]) => 
      'value' in node && typeof node.value === 'function' && node.value.name === 'mesh')?.[0];
    const translateXNodeId = Object.entries(analysis.nodes).find(([_, node]: [string, any]) => 
      'value' in node && typeof node.value === 'function' && node.value.name === 'translateX')?.[0];
  ...
  it('should verify no flattened edge patterns exist', () => {
    const expression = 'mesh(sphere(), material()).translateX(1).rotateY(45).render("test")';
    ...
    const totalFunctionNodes = Object.values(analysis.nodes).filter((node: any) => 
      'value' in node && typeof node.value === 'function').length;
    ...
    Object.entries(analysis.edges).forEach(([nodeId, deps]: [string, any]) => {
      ...
      if (depCount >= totalFunctionNodes - 1) {
        console.log(`⚠️ Potential flattening detected: ${nodeId} has ${depCount} dependencies`);
        hasFlattening = true;
      }
    });
  ...
  it('should analyze detailed conversion log for debugging', () => {
    ...
    const callExpressions = result.conversionLog.filter(entry => entry.astNodeType === 'CallExpression');
    console.log(`\n📞 Call Expressions Found: ${callExpressions.length}`);
    
    callExpressions.forEach(call => {
      console.log(`  ${call.functionResolved} at ${call.position.from}-${call.position.to}`);
    });
