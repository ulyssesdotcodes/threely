...
describe('Lezer vs Eval Converter Equivalence', () => {
  ...
  it('should produce similar graph structure for complex chaining', () => {
    ...
    console.log('Lezer result nodes:', Object.keys(lezerResult.graph.nodes).length);
    console.log('Lezer result edges:', Object.keys(lezerResult.graph.edges || {}).length);
  ...
  it('should handle method chaining properly', () => {
    const result = convertLezerToNodysseus('frame().mult(0.1).abs()', dslContext);
  ...
  it('should handle nested function calls', () => {
    const result = convertLezerToNodysseus('mesh(sphere(1), material())', dslContext);
