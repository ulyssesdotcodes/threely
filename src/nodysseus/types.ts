...
export const isGraph = (graph: any): graph is Graph =>
  graph && graph.hasOwnProperty("nodes");
...
// Node creation and transformation functions
let nodeIdCounter = 0;
 ...
 */
export function constNode<T>(value: T): ConstNode<T> {
  const node: ConstNode<T> = {
    id: generateUniqueId(),
    value,
    compute: () => value,
    dependencies: []
