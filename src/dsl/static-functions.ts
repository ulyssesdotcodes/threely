...
import { MockObject3D, MockGeometry, mockUtils, applyMockToObject3D, createGeometryFromMock, normalizeVector3Like, normalizeEulerLike } from '../three/MockObject3D';
import { getObjectRegistry } from './object3d-chain';

// Static geometry creation functions
export const staticSphere = (radius: number = 1, widthSegments: number = 32, heightSegments: number = 16): MockGeometry => 
  mockUtils.sphereGeometry(radius, widthSegments, heightSegments);

export const staticBox = (width: number = 1, height: number = 1, depth: number = 1): MockGeometry =>
  mockUtils.boxGeometry(width, height, depth);

export const staticCylinder = (radiusTop: number = 1, radiusBottom: number = 1, height: number = 1): MockGeometry =>
...
export const staticMesh = (geometry: MockGeometry, material: THREE.Material): MockObject3D => {
  return {
    geometry: geometry,
    userData: { material }
  };
...
export const staticTranslateX = (mockObject: MockObject3D, distance: number): MockObject3D => {
  if (!mockObject) {
...
export const staticTranslateY = (mockObject: MockObject3D, distance: number): MockObject3D => {
  if (!mockObject) {
    return { geometry: undefined, userData: undefined };
  }
  const currentPos = normalizeVector3Like(mockObject.position || { x: 0, y: 0, z: 0 });
...
export const staticTranslateZ = (mockObject: MockObject3D, distance: number): MockObject3D => {
  if (!mockObject) {
    return { geometry: undefined, userData: undefined };
  }
  const currentPos = normalizeVector3Like(mockObject.position || { x: 0, y: 0, z: 0 });
...
export const staticRotateX = (mockObject: MockObject3D, angle: number): MockObject3D => {
  if (!mockObject) {
    return { geometry: undefined, userData: undefined };
  }
...
export const staticRotateY = (mockObject: MockObject3D, angle: number): MockObject3D => {
  if (!mockObject) {
    return { geometry: undefined, userData: undefined };
  }
  const currentRot = normalizeEulerLike(mockObject.rotation || { x: 0, y: 0, z: 0 });
...
export const staticRotateZ = (mockObject: MockObject3D, angle: number): MockObject3D => {
  if (!mockObject) {
    return { geometry: undefined, userData: undefined };
  }
  const currentRot = normalizeEulerLike(mockObject.rotation || { x: 0, y: 0, z: 0 });
...
export const staticRender = (mockObject: MockObject3D, objectName: string): THREE.Object3D => {
