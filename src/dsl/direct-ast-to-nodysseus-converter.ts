...
export class DirectASTToNodysseusConverter {
  ...
  convert(
    ...
    const rootNodeId = this.convertASTNode(
      tree.topNode,
      ranges,
      context,
      startOffset,
  ...
  private convertASTNode(
    ...
    const actualNodeText = nodeText || this.getNodeText(astNode, context);
    ...
    switch (astNode.name) {
      ...
      case "CallExpression":
        nodeId = this.convertCallExpression(
          astNode,
          ranges,
          context,
          startOffset,
      ...
      case "MemberExpression":
        nodeId = this.convertMemberExpression(
          astNode,
          ranges,
          context,
          startOffset,
      ...
      case "VariableName":
        nodeId = this.convertVariableName(
          astNode,
          ranges,
          context,
          startOffset,
      ...
      case "ObjectExpression":
        nodeId = this.convertObjectExpression(
          astNode,
          ranges,
          context,
          startOffset,
  ...
  private convertCallExpression(
    ...
    if (this.isMethodChain(astNode)) {
      return this.convertMethodChain(
        astNode,
        ranges,
        context,
        startOffset,
  ...
  private convertSingleCall(
    ...
    const uuid = this.nodeIdFromRangeSet(astNode, ranges, startOffset);
    ...
    const argNodeIds = args.map((arg: any, index: number) => {
      const argNodeId = this.convertASTNode(arg, ranges, context, startOffset);
      console.log(
    ...
    if (targetNode) {
      ...
      const targetNodeId = this.convertASTNode(
        targetNode,
        ranges,
        context,
        startOffset,
  ...
  private convertMethodChain(
    ...
    for (let i = 0; i < chainCalls.length; i++) {
      ...
      const uuid = this.nodeIdFromRangeSet(callNode, ranges, startOffset);
      ...
      const argNodeIds = args.map((arg: any, index: number) => {
        const argNodeId = this.convertASTNode(
          arg,
          ranges,
          context,
          startOffset,
  ...
  private convertMemberExpression(
    ...
    logToPanel(
      `🔗 Member expression: ${nodeText || this.getNodeText(astNode, context)}`,
    );
    ...
    if (objectNode) {
      const baseNodeId = this.convertASTNode(
        objectNode,
        ranges,
        context,
        startOffset,
    ...
    return this.convertFirstChild(astNode, ranges, context, startOffset);
  }
  ...
  private convertVariableName(
    ...
    const uuid = this.nodeIdFromRangeSet(astNode, ranges, startOffset);
    const nodeId = uuid || this.generateNodeId(context);
    const variableName = nodeText || this.getNodeText(astNode, context);
  ...
  private convertNumber(
    ...
    const uuid = this.nodeIdFromRangeSet(astNode, ranges, startOffset);
    const nodeId = uuid || this.generateNodeId(context);
    const numberText = nodeText || this.getNodeText(astNode, context);
  ...
  private convertString(
    ...
    const uuid = this.nodeIdFromRangeSet(astNode, ranges, startOffset);
    const nodeId = uuid || this.generateNodeId(context);
    const stringText = nodeText || this.getNodeText(astNode, context);
  ...
  private convertObjectExpression(
    ...
    while (child) {
      if (child.name === "Property") {
        let propertyKey = this.getNodeText(child.getChild("PropertyDefinition"), context);
        ...
        while (propChild) {
          if (propChild.name === "PropertyName") {
            ...
            objectValue[propertyKey] = Number.isNaN(intValue) ? parseFloat(nodeText) : intValue
...
export function convertASTToNodysseus(
  ...
  return converter.convert(sourceCode, ranges, dslContext, startOffset);
}
