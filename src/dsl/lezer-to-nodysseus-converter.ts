...
export class LezerToNodysseusConverter {
   ...
   */
  private convertToNodysseusGraph(
    ...
    const functionalGraph = this.convertASTToFunctionalGraph(
      tree.topNode,
      context,
    );
    logToPanel(
      `🔗 Created functional graph with root node: ${functionalGraph.id}`,
    );
    ...
    logToPanel(
      `🎯 Converted to Nodysseus graph with ${Object.keys(nodysseusGraph.nodes).length} nodes`,
    );
   ...
   */
  private convertASTToFunctionalGraph(
    astNode: any,
    context: ConversionContext,
  ): Node<any> {
    logToPanel(
  ...
  private parseCallChain(astNode: any, context: ConversionContext): any[] {
    ...
    const parseCall = (node: any): void => {
      ...
      const { functionName, targetNode, args } =
        this.extractCallExpressionParts(node, context);
  ...
  private buildCallChain(
    ...
    for (const call of callChain) {
      ...
      logToPanel(
        `🔗 Building call: ${call.functionName}${call.isMethod ? " (method)" : " (function)"} (UUID: ${uuid || "generated"})`,
      );

      if (uuid) {
        logToPanel(
          `✅ Found UUID correlation: ${uuid} for ${call.functionName} at position ${call.astNode.from}-${call.astNode.to}`,
        );
      }
      ...
      if (!dslFunction && call.isMethod && currentNode) {
        ...
        if (
          chainContext &&
          chainContext[call.functionName] &&
          chainContext[call.functionName].fn
        ) {
          dslFunction = chainContext[call.functionName].fn;
          resolvedFunctionName = dslFunction.name || call.functionName;
          logToPanel(
            `🔗 Resolved chain method ${call.functionName} to ${resolvedFunctionName}`,
          );
        }
      }
      ...
      if (!dslFunction) {
        logToPanel(
          `❌ Function '${call.functionName}' not found in DSL context`,
          "error",
        );
        this.logConversion(
          call.astNode,
      ...
      const argNodes = call.args.map((arg: any) =>
        this.convertASTToFunctionalGraph(arg, context),
      );
      ...
      currentNode = createNode(
        refNode,
        dependencies,
        this.getChainContext(call.functionName, context),
      );
    }
    ...
    if (dslValue !== undefined) {
      if (typeof dslValue === "function") {
        ...
        const resultNode = createNode(
          functionRefNode,
          [],
          this.getChainContext(variableName, context),
        );
