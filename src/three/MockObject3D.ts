 ...
 */
export function normalizeVector3Like(vector: Vector3Like): {
  x: number;
  y: number;
  z: number;
} {
 ...
 */
export function normalizeEulerLike(euler: EulerLike): {
  x: number;
  y: number;
  z: number;
  order?: string;
 ...
 */
export function normalizeQuaternionLike(quaternion: QuaternionLike): {
  x: number;
  y: number;
  z: number;
  w: number;
...
export function applyMockToObject3D(
  ...
  if (mock.position !== undefined) {
    ...
    if (
      validateNumber(pos.x, undefined, undefined, "position.x") &&
  ...
  if (mock.rotation !== undefined) {
    ...
    if (
      validateNumber(rot.x, undefined, undefined, "rotation.x") &&
    ...
    if (
      validateNumber(quat.x, undefined, undefined, "quaternion.x") &&
  ...
  if (mock.scale !== undefined) {
    ...
    if (
      validateNumber(scale.x, 0.001, undefined, "scale.x") &&
  ...
  if (
    mock.renderOrder !== undefined &&
    validateNumber(mock.renderOrder, 0, undefined, "renderOrder")
  ) {
...
export const mockPresets = {
  ...
  /** Scaled up object */
  large: (factor: number = 2): MockObject3D => ({
    scale: mockUtils.scale(factor),
  }),

  /** Scaled down object */
  small: (factor: number = 0.5): MockObject3D => ({
    scale: mockUtils.scale(factor),
  }),

  /** Object positioned above ground */
  elevated: (height: number = 1): MockObject3D => ({
