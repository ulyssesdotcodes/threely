 ...
 */
export const convertGraphToNodysseus = <T>(
  rootNode: Node<T>,
  graphId?: string,
): Graph => {
  ...
  const convertNode = (node: Node<any>): string => {
    ...
    const dependencyIds =
      node.dependencies?.map((dep) => convertNode(dep)) ?? [];
...
): Record<string, Record<string, Edge>> => {
 ...
 */
export const convertAndCompareGraphOutputs = <T>(
  rootNode: Node<T>,
): {
  originalOutput: T;
  nodysseusOutput: any;
  ...
  try {
    ...
    if (typeof originalOutput === "object" && originalOutput !== null) {
      outputsMatch =
        JSON.stringify(originalOutput) === JSON.stringify(nodysseusOutput);
    } else {
